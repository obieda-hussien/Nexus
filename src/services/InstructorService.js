...
class InstructorService {
  ...
  // Get all approved instructors with their profiles
  static async getAllInstructors() {
    try {
      ...
      // Enrich instructor data with their stats
      const enrichedInstructors = await Promise.all(
        instructors.map(async (instructor) => {
          const stats = await this.getInstructorStats(instructor.uid);
          return {
            uid: instructor.uid,
            displayName: instructor.displayName || 'مدرب',
            email: instructor.email,
            profilePicture: instructor.profilePicture || null,
            bio: instructor.bio || '',
            specialization: instructor.instructorData?.specialization || instructor.instructorProfile?.specialization || '',
            experience: instructor.instructorData?.experience || instructor.instructorProfile?.experience || '',
            education: instructor.instructorData?.education || instructor.instructorProfile?.education || '',
            socialLinks: instructor.instructorData?.socialLinks || {},
            rating: stats.success ? stats.stats.averageRating : 0,
            totalstudents: stats.success ? stats.stats.totalstudents : 0,
            coursesCount: stats.success ? stats.stats.totalCourses : 0,
            totalReviews: stats.success ? stats.stats.totalReviews : 0
          };
        })
      );
      
      return { success: true, instructors: enrichedInstructors };
    } catch (error) {
