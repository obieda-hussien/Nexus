...
const Footer = () => {
  ...
  const footerSections = [
    {
      ...
      links: [
        ...
        { name: isRTL ? 'Experimental Sciences' : 'Experimental Sciences', href: '#' }
      ]
    },
    {
      title: isRTL ? 'Company' : 'Company',
      links: [
        { name: isRTL ? 'About Us' : 'About Us', href: '#' },
        { name: isRTL ? 'Our Team' : 'Our Team', href: '#' },
        { name: isRTL ? 'Careers' : 'Careers', href: '#' },
    ...
    },
    {
      ...
      links: [
        { name: isRTL ? 'Help Center' : 'Help Center', href: '#' },
        { name: isRTL ? 'FAQ' : 'FAQ', href: '#' },
        { name: isRTL ? 'Contact Us' : 'Contact Us', href: '#' },
        { name: isRTL ? 'System Status' : 'System Status', href: '#' }
      ]
    },
    {
      title: isRTL ? 'Legal' : 'Legal',
      links: [
        { name: isRTL ? 'Privacy Policy' : 'Privacy Policy', href: '#' },
        { name: isRTL ? 'Terms of Service' : 'Terms of Service', href: '#' },
        { name: isRTL ? 'Cookie Policy' : 'Cookie Policy', href: '#' },
        { name: isRTL ? 'Disclaimer' : 'Disclaimer', href: '#' }
      ]
    }
  ];
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      value: isRTL ? 'Cairo, Egypt' : 'Cairo, Egypt',
      href: '#'
    }
  ];

  const socialLinks = [
    { icon: Facebook, href: '#', color: 'hover:text-blue-500', name: isRTL ? 'Facebook' : 'Facebook' },
    { icon: Twitter, href: '#', color: 'hover:text-blue-400', name: isRTL ? 'Twitter' : 'Twitter' },
    { icon: Instagram, href: '#', color: 'hover:text-pink-500', name: isRTL ? 'Instagram' : 'Instagram' },
    { icon: Linkedin, href: '#', color: 'hover:text-blue-600', name: isRTL ? 'LinkedIn' : 'LinkedIn' },
    { icon: Youtube, href: '#', color: 'hover:text-red-500', name: isRTL ? 'YouTube' : 'YouTube' }
  ];

  const stats = [
    { icon: Users, value: '10K+', label: isRTL ? 'students' : 'students' },
    { icon: BookOpen, value: '50+', label: isRTL ? 'Courses' : 'Courses' },
    { icon: Award, value: '25+', label: isRTL ? 'Awards' : 'Awards' }
  ];
  ...
  return (
    <footer className="relative bg-gradient-to-t from-primary-bg to-secondary-bg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          >
            <h3 className="text-3xl font-bold gradient-text mb-4">
              {isRTL ? 'Stay Updated' : 'Stay Updated'}
            </h3>
          ...
          >
            <div className="flex glass rounded-lg p-2">
              <input
                ...
                dir={isRTL ? 'rtl' : 'ltr'}
                aria-label={isRTL ? 'Email address for newsletter subscription' : 'Email address for newsletter subscription'}
              />
              <Button 
                variant="gradient" 
                size="sm"
                aria-label={isRTL ? 'Subscribe to newsletter' : 'Subscribe to newsletter'}
              >
                {isRTL ? 'Subscribe' : 'Subscribe'}
              </Button>
            </div>
          </motion.div>
        </div>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <h4 className="text-white font-semibold mb-4">
              {isRTL ? 'Contact Info' : 'Contact Info'}
            </h4>
                ...
                {socialLinks.map((social, index) => {
                  ...
                  return (
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                      aria-label={`${isRTL ? 'Follow us on' : 'Follow us on'} ${social.name}`}
                    >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between">
            ...
            <div className="flex items-center space-x-4 mt-4 md:mt-0">
              <span className="text-text-secondary text-sm">
                {isRTL ? 'Made with' : 'Made with'}
              </span>
              <span className="text-red-500">❤️</span>
              <span className="text-text-secondary text-sm">
                {isRTL ? 'in Egypt' : 'in Egypt'}
              </span>
            </div>
          </div>
        </div>
      ...
      <motion.button
        ...
        aria-label={isRTL ? 'Scroll to top of page' : 'Scroll to top of page'}
      >
