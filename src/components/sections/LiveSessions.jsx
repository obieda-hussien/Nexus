...
const LiveSessions = () => {
  ...
  const handleJoinLive = (sessionId) => {
    if (!currentUser) {
      toast.error('يجب تسجيل الدخول أولاً');
      return;
    }
    
    toast.success('يتم الانضمام للجلسة...');
    // In a real app, this would open a video call interface
    setTimeout(() => {
      toast('الجلسة ستكون متاحة قريباً');
    }, 2000);
  };

  const handleRegister = (sessionId, sessionTitle) => {
    if (!currentUser) {
      toast.error('يجب تسجيل الدخول أولاً');
      return;
    }
    
    toast.success(`تم التسجيل في ${sessionTitle} بنجاح!`);
    
    // Store registration in localStorage for demo
    const registrations = JSON.parse(localStorage.getItem(`registrations_${currentUser.uid}`) || '[]');
    if (!registrations.some(reg => reg.sessionId === sessionId)) {
      registrations.push({
        sessionId,
        sessionTitle,
        registeredAt: new Date().toISOString()
      });
      localStorage.setItem(`registrations_${currentUser.uid}`, JSON.stringify(registrations));
    }
  };
    ...
    <section className="py-20 bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* Removed mixed-language paragraph */}
        </motion.div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                <div className="flex-1">
                  ...
                  <div className="text-gray-400 text-sm">
                    ...
                    <span>{session.duration}</span> {/* Removed undefined session.durationAr */}
                  </div>
                </div>
