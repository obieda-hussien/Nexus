...
const FirebaseDiagnostic = ({ isOpen, onClose }) => {
  ...
  const addTestResult = (test, status, message, details = null) => {
    setTestResults(prev => [...prev, {
      ...
      message,
      details,
      timestamp: new Date().toLocaleTimeString('en-US') // Changed from 'ar-EG'
    }]);
  };
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      if (currentUser) {
        addTestResult('Authentication', 'success', `User logged in: ${currentUser.email}`, {
          uid: currentUser.uid,
      ...
      if (databaseConnected) {
        ...
        addTestResult('Database Connection', 'error', 'Failed to connect to the database', {
          solution: 'Check Firebase settings and security rules'
        });
      }
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (snapshot.exists()) {
            addTestResult('User Profile', 'success', 'User profile found in the database', {
              data: snapshot.val()
            });
            setDiagnostics(prev => ({ ...prev, userProfileExists: true }));
          } else {
            addTestResult('User Profile', 'warning', 'User profile not found in the database', {
              solution: 'You can create a new profile from the dashboard'
            });
          }
        } catch (error) {
          addTestResult('User Profile', 'error', 'Error reading user profile', {
            error: error.message,
            code: error.code
          });
        }
      ...
      addTestResult('General Error', 'error', 'General error during diagnostics', {
        error: error.message
      });
    }
      ...
      <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className={`bg-gray-800/50 rounded-xl p-4 text-center ${diagnostics.firebaseInit ? 'border border-green-500/30' : 'border border-gray-700/30'}`}>
            ...
            <p className="text-sm text-gray-300">Firebase Init</p>
          </div>
                  ...
                  {result.details && (
                    <div className="mt-2 text-xs text-gray-400">
                      {result.details.solution && (
                        <p className="text-blue-400">Solution: {result.details.solution}</p>
                      )}
