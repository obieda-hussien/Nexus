...
const PayPalPaymentButton = ({ 
  ...
  const initializePayPalButton = async () => {
    try {
      ...
      if (!PayPalCoursePaymentService.isConfigured()) {
        throw new Error('PayPal not configured correctly');
      }
  ...
  const handlePaymentSuccess = async (result) => {
    try {
      ...
      toast.error('An error occurred after payment. Please contact technical support.');
    }
  };
    ...
    <div className={`paypal-payment-container ${className}`}>
      ...
      {paymentSummary && (
        <div className="glass border border-neon-blue/30 bg-neon-blue/5 rounded-xl p-6 mb-6 backdrop-blur-xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white text-lg">Payment Summary</h3>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center py-2">
              <span className="text-text-secondary">Amount in USD:</span>
          ...
          <div className="mt-4 p-3 glass rounded-lg border border-neon-blue/20">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-neon-blue">Fees paid by student - No fees on the platform</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mt-6 glass rounded-xl p-6 border border-glass-border backdrop-blur-xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-white font-semibold mb-3">Secure Payment via PayPal</h4>
            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              ...
              <div className="flex items-center gap-2 text-text-secondary">
                <div className="w-1.5 h-1.5 bg-neon-green rounded-full"></div>
                Automatic Test Environment (Sandbox)
              </div>
              <div className="flex items-center gap-2 text-text-secondary">
                <div className="w-1.5 h-1.5 bg-neon-green rounded-full"></div>
      ...
      <div className="mt-4 text-center">
        <p className="text-text-secondary text-sm mb-2">If you face any issues during payment, contact us at</p>
        <a 
