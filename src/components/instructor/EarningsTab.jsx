...
const EarningsTab = ({ earnings, courses, onSwitchToSettings }) => {
  ...
  useEffect(() => {
    loadWithdrawalData();
  }, [currentUser, earnings]); // Added earnings as dependency
  ...
  const loadWithdrawalData = async () => {
    ...
    try {
      ...
      const netEarnings = (earnings || 0) * 0.85; // After 10% platform fee + 5% tax
      const available = Math.max(0, netEarnings - withdrawn - pending);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold text-white">Earnings Analysis</h2>
          <select
            value={timeRange}
        ...
        {availableBalance > 0 && (
          <div className="mt-6 flex gap-4">
            <button
              ...
              className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 rounded-xl font-semibold flex items-center space-x-2 transition-all"
            >
            ...
            <button
              onClick={() => onSwitchToSettings && onSwitchToSettings()}
              className="bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-xl font-semibold flex items-center space-x-2 transition-all border border-white/20"
            >
      ...
      {/* Earnings Chart */}
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
        <h3 className="text-xl font-semibold text-white mb-6">Monthly Earnings Chart</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
          <h3 className="text-xl font-semibold text-white mb-4">Top Earning Courses</h3>
              ...
              <div key={course.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-purple-200 text-xs">{course.studentsCount} students</p>
                  </div>
                </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
          <h3 className="text-xl font-semibold text-white mb-4">Earnings Details</h3>
          <div className="space-y-4">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <h4 className="text-purple-200 mb-3">Next Payout</h4>
            <div className="bg-white/5 rounded-lg p-4">
              <p className="text-white font-semibold text-lg">{availableBalance.toLocaleString()} EGP</p>
              <p className="text-purple-200 text-sm">
                {availableBalance >= 100 ? 'Available for withdrawal now' : `Minimum withdrawal is 100 EGP`}
              </p>
            </div>
          </div>
          <div>
            <h4 className="text-purple-200 mb-3">Payment Methods</h4>
            <div className="bg-white/5 rounded-lg p-4">
              <p className="text-white font-medium">Bank Account, PayPal, Vodafone Cash</p>
              <p className="text-purple-200 text-sm">
                <button 
                  onClick={() => onSwitchToSettings && onSwitchToSettings()}
