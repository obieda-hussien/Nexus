  ...
  return (
    <div className={`prose prose-invert max-w-none ${className}`}>
      <ReactMarkdown 
        ...
        components={{
          // Style markdown elements for dark theme (LTR)
          h1: ({children}) => (
          ...
          p: ({children}) => (
            <p className="text-purple-100 mb-4 leading-relaxed text-left">
              {children}
            </p>
          ),
          strong: ({children}) => (
          ...
          ul: ({children}) => (
            <ul className="text-purple-100 mb-4 ml-6 list-disc space-y-1">
              {children}
            </ul>
          ),
          ol: ({children}) => (
            <ol className="text-purple-100 mb-4 ml-6 list-decimal space-y-1">
              {children}
            </ol>
          ),
          li: ({children}) => (
          ...
          blockquote: ({children}) => (
            <blockquote className="border-l-4 border-blue-400 pl-4 my-6 bg-blue-500/10 py-3 rounded-r-lg">
          ...
          th: ({children}) => (
            <th className="border border-purple-400/30 bg-purple-500/20 px-3 py-2 text-white font-semibold text-left">
              {children}
            </th>
          ),
          td: ({children}) => (
            <td className="border border-purple-400/30 px-3 py-2 text-purple-100 text-left">
              {children}
            </td>
          ),
          hr: () => (
