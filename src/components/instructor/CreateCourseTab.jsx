...
const CreateCourseTab = ({ onCourseCreated, onCancel }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!currentUser) {
        toast.error('You must login first to create a course');
      ...
      if (!canCreateCourses()) {
        toast.error('You do not have permission to create courses. You must be an Instructor first');
      ...
      if (error.code === 'permission-denied') {
        errorMessage = 'You do not have permission to create a course. Ensure your role is "Instructor" in the system';
        ...
        errorMessage = 'You need to re-login for security';
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            placeholder="Example: Modern Physics Basics"
          />
        </div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-purple-300 focus:outline-none focus:border-purple-400"
          placeholder="Brief description summarizing course content in one line"
          maxLength="120"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows="6"
          className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-purple-300 focus:outline-none focus:border-purple-400"
          placeholder="Write a detailed description about the course, what students will learn, and expected benefits..."
        />
      </div>
...
const CurriculumStep = ({ curriculum, onChange, onNext, onBack }) => {
  ...
  const handleNext = () => {
    if (sections.length === 0) {
      toast.error('Please add at least one section to the lessons');
      return;
    }
    ...
    if (hasEmptyLessons) {
      toast.error('Please complete all lesson titles');
      return;
    }
...
[Link here](https://example.com)"
            ...
            <div>
              ...
              <QuizEditor
                ...
                placeholder="Create an interactive quiz..."
              />
            </div>
          ) : null}
          ...
          <div className="flex items-center space-x-3 space-x-reverse">
            ...
            <div>
              ...
              <p className="text-purple-300 text-sm">
                ...
                {lesson.type === 'quiz' && lesson.quizData?.questions?.length > 0 && ` • ${lesson.quizData.questions.length} Questions`}
              </p>
            </div>
          </div>
...
const PricingStep = ({ data, onChange, onNext, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-semibold text-white">Course Pricing</h3>
...
const ReviewStep = ({ data, onSubmit, onBack }) => {
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-semibold text-white">Review Course Details</h3>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-4">
        ...
        <div>
          <h4 className="text-purple-200 text-sm">Category</h4>
...
const InstructorUpgradePrompt = () => {
  ...
  const handleBecomeInstructor = async () => {
    try {
      ...
      toast.success('🎉 Your account has been upgraded to Instructor successfully! You can now create courses');
      // The page will automatically update due to role change
    } catch (error) {
      console.error('❌ Error becoming instructor:', error);
      toast.error(error.message || 'Failed to upgrade account. Please try again');
    } finally {
      ...
      <div className="text-center">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-6">
          <p className="text-purple-200 mb-4">
            Your current account is registered as <span className="font-semibold text-orange-400">{userProfile?.role || 'student'}</span>
          </p>
          <p className="text-purple-200 mb-4">
            To create and manage courses, you must upgrade your account to <span className="font-semibold text-green-400">Instructor</span>
          </p>
        </div>

        <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-400/30 rounded-xl p-6 mb-6">
          <h3 className="text-lg font-semibold text-white mb-4">Instructor Account Benefits</h3>
          <div className="grid md:grid-cols-2 gap-4 text-right">
            <div className="flex items-center space-x-3 space-x-reverse">
              <div className="w-2 h-2 bg-green-400 rounded-full"></div>
              <span className="text-purple-200">Create unlimited courses</span>
            </div>
            <div className="flex items-center space-x-3 space-x-reverse">
              <div className="w-2 h-2 bg-green-400 rounded-full"></div>
              <span className="text-purple-200">Manage lesson content</span>
            </div>
            <div className="flex items-center space-x-3 space-x-reverse">
              <div className="w-2 h-2 bg-green-400 rounded-full"></div>
              <span className="text-purple-200">Track student progress</span>
            </div>
            <div className="flex items-center space-x-3 space-x-reverse">
              <div className="w-2 h-2 bg-green-400 rounded-full"></div>
              <span className="text-purple-200">Detailed statistics</span>
            </div>
          </div>
        </div>
        ...
        <p className="text-purple-300 text-sm mt-4">
          The upgrade is free and instant - no additional fees
        </p>
      </div>
    </div>
  );
