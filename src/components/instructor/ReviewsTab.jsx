...
const ReviewsTab = ({ courses = [] }) => {
  ...
  const initializeDatabase = async () => {
    try {
      ...
      console.log('Using existing Courses structure for Reviews');
      ...
      setError({
        type: 'initialization',
        message: 'Failed to initialize reviews database',
  ...
  const handleReply = async (reviewId) => {
    ...
    try {
      ...
      toast.error('An error occurred while submitting the reply. Please try again.');
    }
  };
  ...
  const ErrorDisplay = ({ error, onRetry }) => (
    <div className="bg-red-500/10 backdrop-blur-lg rounded-2xl border border-red-500/20 p-8 text-center">
      ...
      <div className="flex justify-center space-x-4">
        <button
          onClick={onRetry}
          className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:from-red-600 hover:to-red-700 transition-all duration-300"
        >
        ...
        {error.type === 'initialization' && (
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all duration-300"
          >
            <RefreshCw className="w-4 h-4" />
  ...
  const InitializationDisplay = () => (
    <div className="bg-blue-500/10 backdrop-blur-lg rounded-2xl border border-blue-500/20 p-8 text-center">
      ...
      <h3 className="text-xl font-semibold text-white mb-2">Initializing Database</h3>
      <p className="text-blue-300 mb-4">Setting up the review system for the first time...</p>
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-purple-300">{stats.totalReviews} Reviews</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-lg p-4 text-center">
            ...
            <div className="text-purple-300 text-sm">Total Reviews</div>
          </div>
          ...
          <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-lg p-4 text-center">
            ...
            <div className="text-yellow-300 text-sm">Overall Rating</div>
          </div>
          <div className="bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-white">{courses.length}</div>
            <div className="text-blue-300 text-sm">Active Courses</div>
          </div>
        </div>
            ...
            if (publishedCourses.length === 0) {
              return (
                <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-12 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-4">No Published Courses</h3>
                  <p className="text-gray-400 text-lg mb-6 max-w-md mx-auto leading-relaxed">
                    You must publish courses first so students can rate them and write reviews
                  </p>
                  <div className="flex items-center justify-center space-x-6 text-sm text-gray-500">
                    <div className="flex items-center space-x-2">
                      <BookOpen className="w-4 h-4" />
                      <span>Publish your courses</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Users className="w-4 h-4" />
                      <span>Attract students</span>
                    </div>
                    <div className="flex items-center space-x-2">
            ...
            if (reviews.length === 0) {
              return (
                <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-12 text-center">
                  ...
                  <p className="text-gray-400 text-lg mb-6 max-w-md mx-auto leading-relaxed">
                    No student has written a review or rating for your courses yet. When students start interacting with the content, their reviews will appear here.
                  </p>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-blue-200 text-sm">
                          Interact with your students and provide high-quality content to encourage them to leave positive ratings.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {review.instructorReply && (
                <div className="mt-4 p-4 bg-blue-500/20 rounded-lg border-l-4 border-blue-400">
                  <div className="flex items-center mb-2">
                    <Reply className="w-4 h-4 text-blue-400 mr-2" />
                    <span className="text-blue-300 font-semibold">Instructor Reply</span>
                    <span className="text-gray-400 text-sm ml-auto">
                      {new Date(review.instructorReply.timestamp).toLocaleDateString('en-US')}
                    </span>
                  </div>
