...
const QuizPlayer = ({ quiz, courseId, lessonId, onComplete, onClose }) => {
  ...
  const handleTimeUp = () => {
    toast.error('The time for the quiz has ended');
    submitQuiz();
  };
  ...
  if (!canStart) {
    return (
      <div className="bg-white rounded-lg p-8 max-w-md mx-auto text-center">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-2">You have exceeded the allowed number of attempts</h3>
        <p className="text-gray-600 mb-4">
          You have made {previousAttempts.length} out of {quiz.maxAttempts} allowed attempts.
        </p>
        <button
          onClick={onClose}
  ...
  if (requiresPassword) {
    return (
      <div className="bg-white rounded-lg p-8 max-w-md mx-auto text-center">
        ...
        <p className="text-gray-600 mb-4">This quiz requires a password to continue</p>
        <input
          type="password"
          value={passwordInput}
      ...
      {quiz.settings?.showProgress && (
        <div className="mb-6">
          ...
          <div className="flex justify-between text-sm text-gray-500 mt-2">
            <span>Progress: {Math.round(getQuestionProgress())}%</span>
            <span>Answered {quiz.questions.filter(q => answers[q.id] !== undefined).length} of {quiz.questions.length}</span>
          </div>
        </div>
      )}
            ...
            <div className="space-y-3">
              <label className="flex items-center space-x-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                <input
                  ...
                  className="w-4 h-4 text-blue-600"
                />
                <span className="text-gray-700 flex-1">True</span>
              </label>
              <label className="flex items-center space-x-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-blue-600"
                />
                <span className="text-gray-700 flex-1">False</span>
              </label>
            </div>
          )}
  ...
  return (
    <div className="bg-white rounded-lg max-w-2xl mx-auto p-8">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to the Quiz</h2>
      ...
      {previousAttempts.length > 0 && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <h4 className="font-semibold text-yellow-900 mb-2">Your Previous Attempts:</h4>
      ...
      <div className="flex space-x-4 justify-center">
        <button
          onClick={onStart}
          className="flex items-center space-x-2 px-8 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-semibold"
        >
    ...
    <div className="bg-white rounded-lg max-w-2xl mx-auto p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="text-center p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-gray-600 text-sm">Correct answers out of {quiz.questions.length}</div>
        </div>
        <div className="text-center p-4 bg-gray-50 rounded-lg">
          <div className="text-2xl font-bold text-gray-900">{attemptNumber}</div>
          <div className="text-gray-600 text-sm">of {maxAttempts} attempts</div>
        </div>
      </div>
