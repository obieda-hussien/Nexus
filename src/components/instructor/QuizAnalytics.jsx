...
const QuizAnalytics = ({ courseId, lessonId, quizTitle }) => {
  const [analytics, setAnalytics] = useState({
    totalAttempts: 0,
    uniqueStudents: 0, // Renamed from uniquestudents
    averageScore: 0,
    passingRate: 0,
    averageTime: 0,
    submissions: [],
  ...
  }, [courseId, lessonId, selectedTimeframe]); // Re-run calculation when timeframe changes
  ...
  const calculateAnalytics = (submissions, analyticsData) => {
    ...
    const passingSubmissions = filteredSubmissions.filter(s => s.score >= 70).length; // Assuming 70 is passing
    ...
    <div className="bg-gray-900 border border-gray-600 rounded-lg p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          title="Average Time"
          value={`${Math.round(analytics.averageTime / 60)} min`}
          color="purple"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-4">
          <h4 className="text-white font-medium mb-2">Passing Rate</h4>
          <div className="flex items-center space-x-2">
          ...
          <p className="text-gray-400 text-sm mt-2">
            {analytics.submissions.filter(s => s.score >= 70).length} of {analytics.totalAttempts} attempts
          </p>
        </div>

        <div className="bg-gray-800 border border-gray-600 rounded-lg p-4">
          <h4 className="text-white font-medium mb-2">Score Distribution</h4>
      ...
      {analytics.questionStats.length > 0 && (
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-4">
          <h4 className="text-white font-medium mb-4">Question Analytics</h4>
              ...
              <div key={stat.questionId} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                <div>
                  ...
                  <div className="text-gray-400 text-sm">
                    {stat.totalAttempts} attempts • {stat.correctRate}% correct
                  </div>
                </div>
                <div className="flex items-center space-x-2">
      ...
      {analytics.submissions.length > 0 && (
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-4">
          <h4 className="text-white font-medium mb-4">Recent Submissions</h4>
      ...
      {analytics.totalAttempts === 0 && (
        <div className="text-center py-8 text-gray-400">
          ...
          <p className="text-sm">Analytics will be shown here once students start taking the quiz</p>
        </div>
      )}
    </div>
  );
