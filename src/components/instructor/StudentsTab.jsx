  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
        <h2 className="text-2xl font-bold text-white mb-4">إدارة الطلاب</h2>
        ...
        <div className="grid md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-purple-200">طلاب نشطين</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-white">
              {students?.filter(s => (s.progress || 0) === 100).length || 0}
            </p>
            <p className="text-purple-200">أكملوا الكورس</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-white">
              {students?.reduce((avg, s) => avg + (s.progress || 0), 0) / Math.max(students?.length || 1, 1) || 0}%
            </p>
            <p className="text-purple-200">متوسط التقدم</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredStudents.length > 0 ? (
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-12 text-center">
            ...
            <p className="text-white text-xl mb-2">لا يوجد طلاب</p>
            <p className="text-purple-200">لم يسجل أي طالب في كورساتك بعد</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4 space-x-reverse">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">تاريخ التسجيل: {enrolledDate}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-6 space-x-reverse">
          ...
          <div className="text-center min-w-20">
            ...
            <p className="text-xs text-gray-400 mt-1">التقدم</p>
          </div>
      ...
      {student.timeSpent && (
        <div className="mt-4 grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
          <div className="text-center">
            <p className="text-sm text-gray-400">الوقت المقضي</p>
            <p className="text-white font-medium">{Math.round(student.timeSpent / 60)} دقيقة</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-gray-400">آخر نشاط</p>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-400">الدروس المكتملة</p>
