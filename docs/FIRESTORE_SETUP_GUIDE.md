# دليل إعداد Cloud Firestore - حل مشكلة عدم ظهور تبويب Rules 🔧

## 🎯 المشكلة المحددة

إذا كنت لا ترى تبويب "Rules" في Cloud Firestore في Firebase Console، فهذا يعني أن **قاعدة بيانات Firestore لم يتم إنشاؤها بعد** أو لم يتم تفعيلها بشكل صحيح.

## 📋 الخطوات المطلوبة لحل المشكلة

### الخطوة 1: التأكد من إعداد Cloud Firestore
1. اذهب إلى [Firebase Console](https://console.firebase.google.com)
2. اختر مشروع `nexus-012`
3. من القائمة الجانبية، اضغط على **"Firestore Database"**
4. إذا رأيت رسالة "Create database"، اضغط عليها

### الخطوة 2: إنشاء قاعدة بيانات Firestore (إذا لم تكن موجودة)

1. **اضغط "Create database"**
2. **اختر "Start in test mode"** (هذا مؤقت - سنغيره لاحقاً)
3. **اختر الموقع الأقرب لك** (مثل `europe-west3` أو `us-central1`)
4. **اضغط "Done"**

### الخطوة 3: الوصول لتبويب Rules
بعد إنشاء قاعدة البيانات:

1. ستظهر صفحة Firestore Database
2. ستجد التبويبات: **Data | Rules | Indexes | Usage**
3. اضغط على تبويب **"Rules"**

### الخطوة 4: تطبيق قواعد الأمان الصحيحة
